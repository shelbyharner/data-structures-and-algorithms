...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(element => {
    Object.keys(element).forEach(house => {
      return (house === 'house')? houses.push(`${element[house]}`):null;
    });
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let answer;
  arr.map(element => {
    let characterCheck = Object.values(element).includes(character);
    let childrenCheck = element.children;
    (characterCheck && (childrenCheck))? answer = true: null;
  });
  return answer;
};
